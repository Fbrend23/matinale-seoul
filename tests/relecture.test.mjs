...
import { MAX_RETRAITS, lotsDe, composerConsigneRelecture, extraireObjet, extraireRelecture, recomposer, rétablirAdresses, rétablirHeures, vérifierRelecture, écarts, rendreRapport } from '../scripts/lib/relecture.mjs';
...
test('un lot rendu dont les items ou les événements ne sont pas des listes est une panne, pas une chute', () => {
  // Le 21 septembre 2026 : `sections` rendu en objet, et la première
  // boucle du script tombait sur « object is not iterable ». Par lots, le
  // même travers donnerait `items` ou `events` en objet.
  const section = ORIGINAL.sections[0];
  assert.throws(() => extraireRelecture(JSON.stringify({ corrections: [], lot: { section: { ...section, items: { 1: section.items[0] } } } })), /`section.items` en object, pas en liste/);
  assert.throws(() => extraireRelecture(JSON.stringify({ ...section, items: null })), /`section.items` en null/);
  assert.throws(() => extraireRelecture(JSON.stringify({ corrections: [], events: { 0: EVENT } })), /`events` en object/);
  assert.throws(() => extraireRelecture(JSON.stringify({ lot: { events: null } })), /`events` en null/);
});
