...
- **`events`** : chaque événement porte `start_date` **et** `end_date`, des
  ...
  `pokemon`, `kpop`, `gaming`, `personnages`, `mode` ; un `summary` de 40 mots
  au plus ; un `venue` en coréen, sauf si `map_url` est donné. Un
...
boutiques éphémères, concerts, expositions, salons — pour six thèmes, et
seulement ceux-là : **anime et manga, Pokémon, K-pop, jeu vidéo, personnages,
mode et marques**. Séoul et sa proche banlieue (Goyang, Seongnam, Incheon),
rien au-delà.

Le thème, c'est ce qui fait venir les gens. Deux d'entre eux demandent une
précision :
...
- **Cherche thème par thème, en coréen.** Une requête par thème, avec les
  ...
  `산리오`, `치이카와`), `패션 브랜드` — combinés à `팝업스토어`, `전시`,
  `콘서트`, `페스티벌`, `서울`, le mois en cours. L'anglais remonte surtout
...
- **Passe ensuite par les sources de l'allowlist qui annoncent les
  ...
  Hypebeast pour la mode, COEX et DDP pour les salons. Une recherche `site:`
...
- Le brief passe avant : compose-le d'abord, cherche les événements ensuite,
  ...
  vingt-cinq de la session — assez pour les six thèmes. L'onglet ne se
